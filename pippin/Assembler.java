...
public class Assembler {
	...
	public static int assemble(File input, File output, StringBuilder error) {
		...
		try (Scanner scan = new Scanner(input)) {
			...
			while(scan.hasNextLine() && retVal == 0) {
				lineNum++;
				String line = scan.nextLine();
				
				if(line.trim().length()==0 && firstBlankLineNum == 0) {
					firstBlankLineNum = lineNum;
				} else if (line.trim().length()==0 && firstBlankLineNum != 0) {
					error.append("Illegal blank line in the source file");
					retVal = firstBlankLineNum;
				} else if (line.trim().length() != 0  && (line.charAt(0)==' '||line.charAt(0)=='\t')) {
		...
		if(retVal==0) {
			for(int i = 0; i < inputText.size() && retVal == 0; i++) {
				...
				//System.out.println(Arrays.toString(parts)); //Prints the input
